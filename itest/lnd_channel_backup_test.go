...
func testChannelBackupRestoreCommitTypes(ht *lntest.HarnessTest) {
	...
	for _, testCase := range testCases {
		...
		// produce a penalty transaction if needed. To do it, use an
...
func runScbForceCloseCommitTypes(ht *lntest.HarnessTest,
	...
	readBackup := func() (chanbackup.Single, []byte) {
		...
		closeTxInputs := single.CloseTxInputs.UnwrapOrFail(ht.T)
		require.NotNil(ht, closeTxInputs.CommitTx)
		require.NotEmpty(ht, closeTxInputs.CommitSig)
	...
	closeTxInputs1 := backupBeforeShutdown.CloseTxInputs.UnwrapOrFail(ht.T)
	balancesBeforeShutdown := outputsAsString(closeTxInputs1.CommitTx)
	...
	closeTxInputs2 := backupAfterShutdown.CloseTxInputs.UnwrapOrFail(ht.T)
	balancesAfterShutdown := outputsAsString(closeTxInputs2.CommitTx)
...
func assertFundsRecoveredStartCarol(ht *lntest.HarnessTest,
	...
	// Upon reconnection, the nodes should detect that Dave is out of sync.
	// Carol should force close the channel using her latest commitment. We
	// detect Dave's tx that was broadcasted manually.
	ht.AssertNumTxsInMempool(1)
