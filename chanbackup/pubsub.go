...
type manualUpdate struct {
	...
	// and the subsequent file updating is successful, nil error is sent.
...
// are processed in another goroutine. If wait=true, the method waits for the
// updates to be fully processed and the file updated on-disk before returning.
func (s *SubSwapper) ManualUpdate(singles []Single, wait bool) error {
	...
	if err != nil {
		return fmt.Errorf("processing of manual update failed: %w", err)
	}
...
func (s *SubSwapper) backupUpdater() {
	...
	for {
		...
		case manualUpdate := <-s.manualUpdates:
			...
			// Send the outcome back to the caller of ManualUpdate.
			// The error channel must have capacity of 1 not to
			// block here.
