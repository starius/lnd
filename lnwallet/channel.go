...
type SignedCommitTxInputs struct {
	...
	// If the channel is of taproot type, this this filled.
...
func GetSignedCommitTx(inputs SignedCommitTxInputs, signer input.Signer) (*wire.MsgTx, error) {
	...
	case inputs.Taproot != nil:
		...
		musigSession := NewPartialMusigSession(
			*localNonce, inputs.OurKey, inputs.TheirKey, signer,
			&inputs.Taproot.FundingOutput, LocalMusigCommit,
		)
...
func (lc *LightningChannel) getSignedCommitTx() (*wire.MsgTx, error) {
	...
	if lc.channelState.ChanType.IsTaproot() {
		inputs.Taproot = &TaprootSignedCommitTxInputs{
			...
			FundingOutput:        lc.fundingOutput,
		}
	}
...
func (lc *LightningChannel) GetLocalForceCloseSummary() (
